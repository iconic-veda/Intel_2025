...
@Language.component("hf_token_pipe")
def hf_token_pipe(doc):
    # Custom component logic (placeholder, adjust as necessary)
    # For this example, we do not perform any actual operations.
    return doc
...
nlp.add_pipe("hf_token_pipe", last=True)
...
class TextAnalyzerService:
    ...
    def __init__(self, model_choice: str = "obi/deid_roberta_i2b2"):
        ...
        self.model_config = [
            {
                "lang_code": "en",
                "model_name": {"spacy": "en_core_web_sm", "transformers": self.model_choice},
            }
        ]
        self.nlp_engine = TransformersNlpEngine(models=self.model_config)
        self.analyzer = AnalyzerEngine(nlp_engine=self.nlp_engine)
